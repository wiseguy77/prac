...
public class AuthenticationController {
  ...
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "로그인 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = IssuedOtpVo.class))),
      @ApiResponse(responseCode = "500", description = "로그인 실패", content = @Content(mediaType = "application/json", schema = @Schema()))})
  @PostMapping("login")
  public IssuedOtpVo logIn(MfaDto mfaDto) {

    return authenticationService.logIn(mfaDto);
  }
  ...
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "OTP 인증 성공 후 JWT 발급", content = @Content(mediaType = "application/json", schema = @Schema(implementation = IssuedJwtVo.class))),
      @ApiResponse(responseCode = "500", description = "OTP 인증 실패", content = @Content(mediaType = "application/json", schema = @Schema()))})
  @PostMapping("otp")
  public IssuedJwtVo verifyOtp(@AuthenticationPrincipal CustomUserDetailsImpl user) {
  ...
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "JWT 재발급 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = IssuedJwtVo.class))),
      @ApiResponse(responseCode = "500", description = "JWT 재발급 실패", content = @Content(mediaType = "application/json", schema = @Schema()))})
  @GetMapping("jwt")
  public IssuedJwtVo reissueJwt(@AuthenticationPrincipal CustomUserDetailsImpl user) {
